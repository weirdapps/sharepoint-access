...
`sharepoint-access` provides `sharepoint-cli`, a TypeScript CLI for SharePoint
Online driven by a browser-captured session. It is the third of three sibling
repos following one identical pattern, and deviating from that pattern needs a
reason recorded here:

| Repo                | CLI              | State dir            | MCP bridge plugin       |
| ------------------- | ---------------- | -------------------- | ----------------------- |
| `outlook-access`    | `outlook-cli`    | `~/.outlook-cli/`    | `mail` (outlook-bridge) |
| `teams-access`      | `teams-cli`      | `~/.teams-cli/`      | `chat` (teams-bridge)   |
| `sharepoint-access` | `sharepoint-cli` | `~/.sharepoint-cli/` | `files` (planned)       |
...
These were measured against the live tenant on 2026-08-08. Do not design around
assumptions that contradict them without re-probing first.

- The tenant issues **no SharePoint Bearer**. Auth is cookies only
  (`FedAuth` + `rtFa`).
- **Microsoft Graph is unusable.** `graph.microsoft.com` needs a Graph-audience
  Bearer. The tenant additionally returns 403 on the SharePoint-hosted
  `/_api/v2.0` mirror. Classic SPO REST is the only viable surface.
- **Writes need `X-RequestDigest`** from `POST /_api/contextinfo`, valid 1800s.
  Cookie-authenticated POSTs are rejected without it.
- **A stale digest returns 403, not 401.** Discriminate on the body string
  `security validation for this page is invalid` before reporting a permissions
  error, or every expired write looks like an access problem.
...
Test files use the **`.spec.ts`** suffix (enforced in `vitest.config.ts`),
matching `outlook-access`. `teams-access` uses `.test.ts`. Do not mix.

The PII gauntlet blocks the tenant hostname in tracked files. Write
`<tenant>.sharepoint.com` in docs, never the real host.

## Exit codes

`src/util/exit-codes.ts` mirrors `teams-access` exactly for 0-6, so all three
CLIs can be handled identically by cron wrappers: 4 is always "re-authenticate",
5 is always "upstream misbehaved". Code 7 (`NotImplemented`) is temporary and
must be removed once phase 3 lands.
