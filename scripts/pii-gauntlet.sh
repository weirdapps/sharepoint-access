...
#    carried tenant, overridden, envvar, dummy, placeholder, foo, bar and the
#    bare letters y and z. Grepped over the tracked tree, none of them appears
#    anywhere; each was a live exemption for a tenant label nobody had written.
#    What IS used here: `x.sharepoint.com` (68 occurrences, the host-suffix and
#    SSRF fixtures in test_scripts/), and `a.sharepoint.com` / `b.sharepoint.com`
#    (4 and 6, the two-host precedence fixtures in test_scripts/config.spec.ts
#    and the phase-1-3 plan). Those three stay; the other nine are gone.
#
...
PLACEHOLDER_TENANT="$PLACEHOLDER"'|your[-_.]?tenant|<[a-z0-9_-]+>(-my)?\.sharepoint|(^|[^a-z0-9.-])(a|b|x)(-my)?\.sharepoint'
