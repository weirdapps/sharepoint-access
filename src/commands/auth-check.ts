...
type Prober = Pick<SharepointClient, 'getJson' | 'postJson'>;
...
export async function runAuthCheck(client: Prober): Promise<AuthCheckResult> {
  const probes = await Promise.all([
    timed('read', () => client.getJson('/_api/web?$select=Title')),
    // contextinfo is the write dependency: no digest means no writes, even
    // though every read still works.
    timed('write', () => client.postJson('/_api/contextinfo')),
    timed('search', () => client.getJson("/_api/search/query?querytext='test'&rowlimit=1")),
  ]);
